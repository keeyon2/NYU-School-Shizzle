...
\textbf{Due: Friday, February 6, 2015, at 4pm (Submit via NYU Classes)}
...
$$\del_\theta J(\theta) = X^T X \theta - X^T y $$
...
$$J(\theta+\eta(\frac{\del_\theta J(\theta)}{|\del_\theta J(\theta)|}))-J(\theta)$$
\item Write down the expression for updating $\theta$ in the gradient descent
algorithm. Let $\eta$ be the step size. 
$$\theta := \theta -n(\frac{\del_\theta J(\theta)}{|\del_\theta J(\theta)|})$$
...
\textbf{0.1 was a good step size that took about 60 steps to converge.  0.01 was not as good, This one took about 600 steps.  This was moving 10 times as slow and took a magnitude of 10 more steps.  Intuition shows that 0.1 probably is not taking too large of steps at all, so I tried a larger step size.  After moving the step size to 1, I had it converge in roughly 6 steps, which shows that we did not have the issue of moving too far with a step size of 1.  I have tried going larger, and have tried a step size of 3, but we never converge, meaning 3 is too large of a step that will not hit the min.}
...
$$ \del_\theta J(\theta) = (X^TX + \lambda I)^{-1} X^T Y$$
$$\theta := \theta -n(\frac{\del_\theta J(\theta)}{|\del_\theta J(\theta)|})$$
$$\theta := \theta -n(\Delta_\theta J(\theta))$$
...
\\ \\
\textbf{If we use a large $B$, then the bias term will add a massive amount to our total error. This means that when gradient descent converges while using a large $B$, the weight for the bias will always be quite small, effectively making it so our bias term never taken into account in the regression. We can make that regularization weak by instead of using $B = 1$, we can make $B$ start as a very small number.  We can set $B = .1, B = 0.01, B = 0.001$, etc}
