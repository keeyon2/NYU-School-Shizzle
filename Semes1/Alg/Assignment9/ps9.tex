...
\ifnum\me<2
\begin{solution} \\
The way to solve with two BFS calls is you first run a BFS with the red node as your source and for each node save the distance as $Red\ Distance$.  You then run a BFS with the blue node as your source, and have each node save the distance labeled $Blue\ Distance$.
\\
\\
After that you compare every nodes $Red\ Distance$ with their $Blue\ Distance$.  If the $Blue\ Distance < Red\ Distance$, you then change the node's color to Blue.  If the $Red\ Distance \leq Blue\ Distance$, then you change the node's color to Red.
\end{solution}
\fi
...
If $$ CN.Color == Red\ and\ AN.Color == blue$$ $$and$$ $$CN.Distance + 1 \leq AN.Distance$$ then we change $AN.Color$ to red, and we enqueue $AN$. \\

If $$ CN.Color == Blue\ and\ AN.Color == blue$$ $$and$$ $$CN.Distance + 1 < AN.Distance$$ then we change $AN.Color$ to red, and we enqueue $AN$. \\
\\ \\
The reason the change from red to blue happens when distance is $\leq$ and the blue to red only happens with distance is $<$ is because red is analyzed first.  If the problem was changed to analyze blue first, we would change this. \\
