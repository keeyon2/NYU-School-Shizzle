...
$n^{log_4(8)}$, which reduces down to $n^{1.5}$ which equals $n\ log(n)$
...
\\
\begin{enumerate}
	\item[1. ] $T(n) = 8(c*\frac{n}{4}*\ log(\frac{n}{4}) + 1) + n$
	\item[2. ] $T(n) = 2nc*\ log(\frac{n}{4}) + 8 + n$
	\item[3. ] $T(n) = (2nc*\ log(n)) - (2nc*\ log(4)) + 8 + n$
	\item[4. ] $T(n) = cn\ log(n) - cn + n$
	\item[5. ] $T(n) \leq cn\ log(n) $
	\item[6. ] $T(n) \leq  g(n)$
\end{enumerate}
...
When we sum those all together, we get $2n\ log(n) + n\ log(n)$ which equals {\LARGE $3n\ log(n)$}
